  ...
  useEffect(() => {
    const initGrowCST = async () => {
      ...
      const getCST = await axios({
        url: "http://localhost:3005/getRSTGrow",
            ...
            return (
              <>
                <div className="div-Area-Button-Project">
                  <button
                    onClick={() => {
                      Cookies.set("RST", data.ID_Stem);
                    }}
                  >
                    <Link className="Link-Underline-Color" to="/growRSTDetail">
                      {data.ID_Stem}
                    </Link>
                  </button>
                </div>
      ...
      <div className="Body-GrowSST">
        ...
        <div className="Head-Farm">
          <div className="div-Head-Left-Side">
            <p className="P-Farm">การเจริญเติบโต ขั้นตอน RST</p>
          </div>
